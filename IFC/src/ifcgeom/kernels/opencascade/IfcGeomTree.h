...
class tree {
                ...
                for (int j = bvh->BegPrimitive(i); j <= bvh->EndPrimitive(i); ++j) {
                    ...
                    if (intersectRayTriangle(ray_origin, ray_vector, ta, tb, tc, at, au, av, false)) {
                        ...
                        if (at > 0 && at < edge_length) {
                            ...
                            if (
                                is_point_on_line(int_vec, ta, tb) || is_point_on_line(int_vec, ta, tc) || is_point_on_line(int_vec, tb, tc) || (ta - int_vec).Magnitude() < 1e-4 || (tb - int_vec).Magnitude() < 1e-4 || (tc - int_vec).Magnitude() < 1e-4) {
                                continue;
                            }
        ...
        for (const auto& pair : bvh_clashes) {
            ...
            for (int i = bvh_a->BegPrimitive(bvh_a_i); i <= bvh_a->EndPrimitive(bvh_a_i); ++i) {
                ...
                for (int v_id : tri) {
                    ...
                    if (is_point_in_shape(v, bvh_b, tris_b, verts_b) && is_point_in_shape(v, bvh_b, tris_b, verts_b, true)) {
                        points_in_b.push_back(v);
                ...
                if (points_in_b.empty()) {
                    ...
                    if (protrusion == -std::numeric_limits<double>::infinity() && pierce != max_protrusion) {
                        std::array<
                            std::tuple<double, std::array<double, 3>, std::array<double, 3>>,
                            3>
                            pierce_results = {
                ...
                for (const auto& bvh_b_i : bvh_b_is) {
                    for (int j = bvh_b->BegPrimitive(bvh_b_i); j <= bvh_b->EndPrimitive(bvh_b_i); ++j) {
                        ...
                        if (trianglesIntersect(v1_a_vec, v2_a_vec, v3_a_vec, v1_b_vec, v2_b_vec, v3_b_vec, int1, int2, !allow_touching)) {
                            ...
                            if (
                                !is_point_on_line(int1, v1_a_vec, v2_a_vec) && !is_point_on_line(int1, v1_a_vec, v3_a_vec) && !is_point_on_line(int1, v2_a_vec, v3_a_vec)) {
                                if (
                                    (v1_b_vec - int1).Magnitude() > 1e-4 && (v2_b_vec - int1).Magnitude() > 1e-4 && (v3_b_vec - int1).Magnitude() > 1e-4) {
                                    return {2, tA, tB, 0, {int1.X(), int1.Y(), int1.Z()}, {int2.X(), int2.Y(), int2.Z()}};
                                }
                            }

                            if (
                                !is_point_on_line(int1, v1_b_vec, v2_b_vec) && !is_point_on_line(int1, v1_b_vec, v3_b_vec) && !is_point_on_line(int1, v2_b_vec, v3_b_vec)) {
                                if (
                                    (v1_a_vec - int1).Magnitude() > 1e-4 && (v2_a_vec - int1).Magnitude() > 1e-4 && (v3_a_vec - int1).Magnitude() > 1e-4) {
                                    return {2, tA, tB, 0, {int1.X(), int1.Y(), int1.Z()}, {int2.X(), int2.Y(), int2.Z()}};
                                }
                            }

                            if (
                                !is_point_on_line(int2, v1_a_vec, v2_a_vec) && !is_point_on_line(int2, v1_a_vec, v3_a_vec) && !is_point_on_line(int2, v2_a_vec, v3_a_vec)) {
                                if (
                                    (v1_b_vec - int2).Magnitude() > 1e-4 && (v2_b_vec - int2).Magnitude() > 1e-4 && (v3_b_vec - int2).Magnitude() > 1e-4) {
                                    return {2, tA, tB, 0, {int2.X(), int2.Y(), int2.Z()}, {int1.X(), int1.Y(), int1.Z()}};
                                }
                            }

                            if (
                                !is_point_on_line(int2, v1_b_vec, v2_b_vec) && !is_point_on_line(int2, v1_b_vec, v3_b_vec) && !is_point_on_line(int2, v2_b_vec, v3_b_vec)) {
                                if (
                                    (v1_a_vec - int2).Magnitude() > 1e-4 && (v2_a_vec - int2).Magnitude() > 1e-4 && (v3_a_vec - int2).Magnitude() > 1e-4) {
                                    return {2, tA, tB, 0, {int2.X(), int2.Y(), int2.Z()}, {int1.X(), int1.Y(), int1.Z()}};
                                }
                            }
                        }
  ...
  public:
    ...
    std::vector<clash> clash_intersection_many(
        const std::vector<T>& set_a, const std::vector<T>& set_b, double tolerance = 0.002, bool check_all = true) const {
                ...
                for (auto& task : tasks) {
                    ...
                    if (is_manifold_.find(task.a)->second) {
                        ...
                        if (intersection.clash_type != -1) {
                            ...
                            if (
                                !has_clash || (result.clash_type == 1 && intersection.clash_type == 0) || (result.clash_type == intersection.clash_type && intersection.distance > result.distance)) {
                                has_clash = true;
                                result = intersection;
                            }
                        }
    ...
    std::vector<clash> clash_collision_many(
        const std::vector<T>& set_a, const std::vector<T>& set_b, bool allow_touching = false) const {
        std::vector<clash_task> task_queue;
        std::vector<clash> results;
    ...
    std::vector<clash> clash_clearance_many(
        const std::vector<T>& set_a, const std::vector<T>& set_b, double clearance = 0.05, bool check_all = false) const {
        std::vector<clash_task> task_queue;
        std::vector<clash> results;
...
class tree : public impl::tree<const IfcUtil::IfcBaseEntity*> {
  public:
    ...
    //             std::transform(verts.begin(), verts.end(), verts_float.begin(), [](double val) { return static_cast<float>(val); });
    ...
    //                     if (std::abs(colour[0] - static_cast<float>(material->diffuse.ccomponents()[0])) < tolerance && std::abs(colour[1] - static_cast<float>(material->diffuse.ccomponents()[1])) < tolerance && std::abs(colour[2] - static_cast<float>(material->diffuse.ccomponents()[2])) < tolerance && std::abs(colour[3] - alpha) < tolerance) {
    ...
    //                     colours.push_back({static_cast<float>(material->diffuse.ccomponents()[0]),
    ...
    void add_element(IfcGeom::TriangulationElement* elem) {
        ...
        for (int i = 0; i < elem_faces.size(); i += 3) {
            ...
            if (cross_product.Magnitude() > Precision::Confusion()) {
                ...
                original_tris_index++;
                original_tris.push_back({elem_faces[i], elem_faces[i + 1], elem_faces[i + 2]});
    ...
    std::vector<IfcGeom::ray_intersection_result> select_ray(const gp_Pnt& p0, const gp_Dir& d, double length = 1000.) const {
        ...
        for (auto& c : candidates) {
            ...
            for (int i = 1; i <= dss.NbSolution(); ++i) {
                ...
                gp_Vec V;
                prop.Normal(v, w, P, V);
                ordered.insert({u, {u, sidx, c, {P.X(), P.Y(), P.Z()}, {V.X(), V.Y(), V.Z()}, d.XYZ().Dot(p0.XYZ() - P.XYZ()), V.Dot(d)}});
            }
        }
